"""
Runtime Safety Guards for Generated Manim Code

This module provides utilities that can be embedded directly into generated
Manim code to provide runtime safety checks and automatic corrections.
"""

from typing import Any, List, Tuple, Optional, Union


def generate_runtime_safety_code() -> str:
    """
    Generate Python code for runtime safety utilities that can be embedded
    in generated Manim scenes.
    
    Returns:
        String containing Python code for safety utilities
    """
...
def generate_scene_wrapper_code() -> str:
    """
    Generate a Scene wrapper class with built-in safety features
    
    Returns:
        String containing Scene wrapper code
    """
...
def generate_validation_decorators() -> str:
    """
    Generate decorator functions for runtime validation
    
    Returns:
        String containing decorator code
    """
...
def get_embedded_safety_code() -> str:
    """
    Get complete embedded safety code for generated Manim scenes
    
    Returns:
        Complete safety code as a string
    """
    return (
        ...
        "\n\n" +
        generate_validation_decorators()
    )


def create_safe_code_template(scene_name: str = "VideoScene", 
                            title: str = "Sample Animation") -> str:
    """
    Create a complete Manim scene template with embedded safety features
    ...
    Returns:
        Complete Manim scene code with safety features
    """
    
    safety_code = get_embedded_safety_code()
...
class {scene_name}(SafeScene):
    def construct(self):
        """Main animation construction with built-in safety"""
        
        # Enable safety features
        self.enable_safety()
        
        # Example: Create title with automatic safety
        title = Text("{title}", font_size=48, color=BLUE)
        scale_to_fit_safe(title)
        safe_position(title, SAFE_TOP)
        
        # Example: Create and arrange multiple elements safely
        elements = [
            Text("Element 1", font_size=36),
            Text("Element 2", font_size=36), 
            Text("Element 3", font_size=36)
        ]
        
        # Arrange elements with automatic spacing and bounds checking
        element_group = safe_arrange(elements, DOWN, buff=0.5)
        
        # Add to scene with safety checks
        self.safe_add(title)
        self.safe_add(element_group)
        
        # Animate with safety checks
        self.safe_play(Write(title), run_time=2)
        self.safe_play(Write(element_group), run_time=3)
        
        self.wait(2)
        
        # Clean exit
        self.safe_play(FadeOut(title), FadeOut(element_group))
...
def inject_safety_into_existing_code(manim_code: str) -> str:
    """
    Inject safety utilities into existing Manim code
    
    Args:
        manim_code: Existing Manim scene code
        
    Returns:
        Modified code with safety features injected
    """
    
    safety_code = get_embedded_safety_code()
    
    # Split the existing code
    lines = manim_code.split('\n')
    
    # Find import section and class definition
    import_end_idx = 0
    class_start_idx = 0
    
    for i, line in enumerate(lines):
        if line.strip().startswith('from manim import') or line.strip().startswith('import'):
            import_end_idx = max(import_end_idx, i + 1)
        elif line.strip().startswith('class ') and 'Scene' in line:
            class_start_idx = i
            break
    
    # Inject safety code after imports
    modified_lines = (
    ...
    # Replace Scene inheritance with SafeScene
    for i, line in enumerate(modified_lines):
        if 'class ' in line and '(Scene)' in line and 'SafeScene' not in line:
            modified_lines[i] = line.replace('(Scene)', '(SafeScene)')
            break
    
    # Add safety calls to common operations
    result_lines = []
    for line in modified_lines:
        stripped = line.strip()
        
        # Replace common operations with safe versions
        if stripped.startswith('self.add('):
            result_lines.append(line.replace('self.add(', 'self.safe_add('))
        elif stripped.startswith('self.play('):
            result_lines.append(line.replace('self.play(', 'self.safe_play('))
        elif '.move_to(' in line and not 'safe_position' in line:
            # Wrap move_to calls with safety
            result_lines.append(line.replace('.move_to(', '.move_to(safe_position('))
        else:
